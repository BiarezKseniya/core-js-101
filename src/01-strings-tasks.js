...
function getRectangleString(width, height) {
  let string = '';
  const row = '│'.concat(' '.repeat(width - 2), '│\n');
  string.concat('┌', '─'.repeat(width - 2), '┐\n');
  string = string.concat('┌', '─'.repeat(width - 2), '┐\n', row.repeat(height - 2), '└', '─'.repeat(width - 2), '┘\n');
  return string;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (firstHalf.indexOf(str[i]) >= 0) {
      res += secondHalf[firstHalf.indexOf(str[i])];
    } else {
      res += firstHalf[secondHalf.indexOf(str[i])];
    }
  }
  return res;
}
